...
class ProducerTest extends JUnitSuite {
  ...
  def testZKSendToNewTopic() {
    ...
    try {
      ...
      producer.send(new ProducerData[String, String]("new-topic", "test", seqAsJavaList(Array("test1"))))
      Thread.sleep(100)
      producer.send(new ProducerData[String, String]("new-topic", "test", seqAsJavaList(Array("test1"))))
      Thread.sleep(100)
  ...
  def testPartitionedSendToNewTopic() {
    ...
    producer.send(new ProducerData[String, String]("test-topic1", "test", seqAsJavaList(Array("test1"))))
    Thread.sleep(100)
    ...
    producer1.send("test-topic1", new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                           messages = seqAsJavaList(Array(new Message("test".getBytes())))))
    Thread.sleep(100)

    // wait for zookeeper to register the new topic
    producer.send(new ProducerData[String, String]("test-topic1", "test1", seqAsJavaList(Array("test1"))))
    Thread.sleep(100)
    producer.close
